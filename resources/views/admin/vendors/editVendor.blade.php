                            ...
                            <div class="card-content collapse show">
                                <div class="card-body">
                                    <form class="form" action="{{route('admin.vendors.update',$vendor->id)}}" method="POST"
                                          ...
                                          @csrf
                                        <div class="form-body">
                                            <img src="{{asset('assets/'.$vendor->logo)}}" width="100%" height="500px" alt="logo" class="text-center" style="margin: auto" />
                                                ...
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        @foreach ($activeCatigories as $category)
                                                                <option value="{{$category->id}}" {{($vendor ->catigory_id == $category->id ? 'selected' : '')}}> {{$category ->name}}</option>
                                                        @endforeach
                                                        @endisset
                                                    </optgroup>
                                                </select>
                                            ...
                                            <div class="row">
                                                ...
                                                <div class="col-md-6">
                                                    <div class="form-group">
                                                        ...
                                                        <input type="text" value="{{$vendor -> phone}}" id="phone"
                                                               class="form-control"
                                    ...
                                    <input type="hidden" name="" value="{{$vendor->lat}}" id="latitude">
                                    <input type="hidden" name="" value="{{$vendor->lng}}" id="longitude">
                                </div>
                            </div>
                        </div>
                    </div>
...
if (navigator.geolocation) {
    navigator.geolocation.getCurrentPosition(function (position) {
        ...
        // if(Latitude != document.getElementById('latitude').value ||  Longitude != document.getElementById('longitude').value ){
        //     alert('The current address is not same the old address');
        // }
    });
} else {
    ...
    //     // var flag = confirm('الموقع الحالي للمتجر لا يطابق الموقع المسجل في قاعدة البيانات ...هل تحديث موقع المتجر للعنوان الجديد);
    //     // if(flag){
    ...
    interv = setInterval(function () {
        if (Latitude != null || Longitude != null) {
            ...
            var latAndLong = { lat:document.getElementById('latitude').value , lng: document.getElementById('longitude').value };
            var zoomLevel = 14;
